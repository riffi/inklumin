...
const getExportData = async (db: BlockAbstractDb, configurationUuid: string) => {
  ...
  const config = await db.bookConfigurations.where("uuid").equals(configurationUuid).first();

  if (!config) return null;

  const blocks = (
    await db.blocks.where("configurationUuid").equals(configurationUuid).toArray()
  ).map(excludeId);
  ...
  const relations = (
    await db.blocksRelations.where("configurationUuid").equals(configurationUuid).toArray()
  ).map(excludeId);
