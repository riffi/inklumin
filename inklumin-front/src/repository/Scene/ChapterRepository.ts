...
export const create = async (
  db: BookDB,
  chapterData: Pick<IChapter, "title">,
...
export const remove = async (db: BookDB, chapterId: number): Promise<void> => {
  ...
  await db.scenes.where("chapterId").equals(chapterId).modify({ chapterId: null });
...
export const reorderChapters = async (
  ...
  await db.transaction("rw", db.chapters, async () => {
    await Promise.all(updates.map((update) => db.chapters.update(update.id, update.changes)));
  });
