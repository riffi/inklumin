...
import { ActionIcon, Box, Group, Stack, Table, Text, TextInput } from "@mantine/core";
    ...
    () =>
      getReverseRelations(tableName)
        .map((rel) => {
          ...
          const count =
            sourceTable?.data.filter((entry: any) => entry[rel.sourceField] === value).length || 0;
  ...
  return (
    <>
      ...
      {reverseLinks.length > 0 && (
        <ReverseRelationLinks links={reverseLinks} onClick={onReverseRelationClick} />
      )}
    </>
  );
};

const ReverseRelationLinks = ({
  links,
  onClick,
}: {
  ...
  <Table.Tr>
    ...
    >
      <Group pt={5}>
        <IconLink color={"#aaa"} size={16}></IconLink>
