...
import { Box, List, LoadingOverlay, ScrollArea, Table, Tabs, Text, Title } from "@mantine/core";
...
export const DbViewer = () => {
  ...
  const handleValueClick = async (key: string, value: string) => {
    ...
    if (relation) {
      ...
      try {
        ...
        const relatedData = await db[relation.table].where(relation.field).equals(value).toArray();
        ...
        setHistory((prev) => [
          ...prev,
          { table: { name: relation.table, data: relatedData }, filter: { field: key, value } },
        ]);
  ...
  const handleReverseRelationClick = (tableName: TableName, field: string, value: string) => {
      ...
      <Tabs value={activeTab} onChange={handleTabChange}>
        ...
        <Tabs.Panel value={activeTab} pt="md">
          ...
          {selectedTable ? (
            ...
            <TableList
              tables={activeTab === "book" ? bookTables : configTables}
