...
export const useConfigurationManager = () => {
  ...
  const removeConfiguration = async (configuration: IBookConfiguration) => {
    ...
    const paramsPossibleValues = await configDatabase.blockParameterPossibleValues
      .where("parameterUuid")
    ...
    await configDatabase.blockParameterPossibleValues.bulkDelete(
      paramsPossibleValues.map((pv) => pv.id)
    );
  ...
  const importConfiguration = async (data: any) => {
    if (
      (await configDatabase.bookConfigurations
        .where("uuid")
        .equals(data.configuration.uuid)
        .count()) > 0
    ) {
      notifications.show({
        title: "Ошибка",
    ...
    await configDatabase.transaction(
      "rw",
      [
