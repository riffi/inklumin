...
import { IconEdit, IconPlus, IconQuestionMark, IconTrash } from "@tabler/icons-react";
      ...
      <Checkbox
        ...
        label={
          !props.isMultiple
            ...
            : `${props.fullParam.instance.value === 1 ? "Checked" : "Unchecked"}`
        }
...
export const ParameterList = ({
  ...
  const groupedParams = fullParams.reduce(
    (acc, param) => {
  ...
  return (
    <>
      <Stack gap="lg" className={classes.parametersStack}>
        {" "}
        {/* Increased gap for groups */}
        {Object.entries(groupedParams).map(([blockParameterUuid, group]) => {
          ...
          if (parameter.allowMultiple) {
            ...
            function renderHeader() {
              // This specific renderHeader is for single, non-multiple params
              return (
                <Group
            ...
            return (
              <Box
                key={`instance-${blockParameterUuid}-${fullParam.instance.id}`}
