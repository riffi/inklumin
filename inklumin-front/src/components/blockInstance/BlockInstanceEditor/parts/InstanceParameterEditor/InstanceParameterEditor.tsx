...
import { AddParameterModal } from "@/components/blockInstance/BlockInstanceEditor/parts/InstanceParameterEditor/modal/AddParameterModal";
import { ParameterList } from "@/components/blockInstance/BlockInstanceEditor/parts/InstanceParameterEditor/parts/ParameterList";
...
const ParameterGroupsTabs = ({ groups, currentGroup, onChange, children }) => (
  <Tabs
    ...
    onChange={(value) => onChange(groups.find((g) => g.uuid === value))}
  >
...
export const InstanceParameterEditor = ({
  ...
  const fullParams: FullParam[] = parameterInstances
    ? parameterInstances
        ?.map((instance) => ({
  ...
  const handleUpdateParameterValue = async (
