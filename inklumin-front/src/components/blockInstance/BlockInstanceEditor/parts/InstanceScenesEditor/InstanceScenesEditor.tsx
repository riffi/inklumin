...
export const InstanceScenesEditor: React.FC<IInstanceScenesEditorProps> = ({
  blockInstanceUuid,
  blockUuid,
}) => {
  ...
  const allScenesCollection = useLiveQuery(() => SceneRepository.getAll(bookDb), [], undefined);

  const allChapters = useLiveQuery(() => ChapterRepository.getAll(bookDb), [], undefined);

  const [linkedScenesDetails, setLinkedScenesDetails] = useState<
    Array<{ link: IBlockInstanceSceneLink; scene: IScene | undefined }>
  >([]);
  useEffect(() => {
  ...
  useEffect(() => {
    if (allChapters && allScenesCollection && linkedSceneLinks) {
      ...
      allChapters.forEach((chapter) => {
        ...
        if (
          chapterIdStr &&
          scenesByChapterId[chapterIdStr] &&
          scenesByChapterId[chapterIdStr].length > 0
        ) {
          nodes.push({
  ...
  if (
    linkedSceneLinks === undefined ||
    allScenesCollection === undefined ||
  ...
  return (
    <Box p="md">
      ...
      {linkedScenesDetails.length === 0 && <Text>Нет привязанных сцен.</Text>}
      ...
      >
        ...
        />
        {modalLoading ? (
          <Text>Загрузка доступных сцен...</Text>
        ) : (
                  ...
                  <Box ml="md" mt="xs">
                    {node.scenes.length > 0 ? (
                      ...
                      <Text size="sm" c="dimmed" ml="lg">
                        {searchTerm.trim()
                          ? "Нет совпадений в этой главе"
                          : "Нет доступных сцен в этой главе."}
                      </Text>
                    )}
                  </Box>
                </Collapse>
            ...
            {filteredTreeData.length === 0 && !modalLoading && (
              <Text>
                {searchTerm.trim()
                  ...
                  : "Нет доступных сцен для привязки."}
              </Text>
            )}
          </ScrollArea>
        )}
