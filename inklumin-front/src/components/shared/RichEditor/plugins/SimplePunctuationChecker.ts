...
const SimplePunctuationChecker = Extension.create({
  ...
  addProseMirrorPlugins() {
              ...
              if (similarWords.length > 0) {
                decorations.push(
                  Decoration.inline(from, to, {
                    style:
                      "background-color: #fff3bf; border-bottom: 1px dashed #ffd43b; padding: 0 1px;",
                    "data-repeated": "true",
                  })
                );
    ...
    const createMergedDecorations = (doc) => {
      return DecorationSet.create(doc, findRepeatedWords(doc).concat(findPunctuationErrors(doc)));
    };
    ...
    return [
      new Plugin({
        state: {
          ...
          apply: (tr, oldState) => (tr.docChanged ? createMergedDecorations(tr.doc) : oldState),
        },
        props: {
          decorations(state) {
