...
export const useSceneLayout = () => {
  ...
  const getLinkedBlockInstances = async (sceneId: number) => {
    ...
    const [blocks, instances] = await Promise.all([
      Promise.all(blockUuids.map((uuid) => BlockRepository.getByUuid(bookDb, uuid))),
      Promise.all(
        instanceUuids.map((uuid) => BlockInstanceRepository.getByUuid(bookDb, uuid))
      ).then((arr) => arr.flat()),
    ]);

    // Фильтруем блоки по showInSceneList === 1
    const filteredBlocks = blocks.filter((b): b is IBlock => !!b && b.showInSceneList === 1);

    const result = filteredBlocks.map((block) => ({
      block,
      instances: instances.filter((instance) =>
        links.some((l) => l.blockUuid === block.uuid && l.blockInstanceUuid === instance.uuid)
      ),
    }));
  ...
  const getScenesWithBlockInstances = async (
    scenesToShow?: IScene[]
  ): Promise<ISceneWithInstances[]> => {
