...
async function extractChaptersAndScenesFromEpub(bookEpub: Book) {
  ...
  for (const tocItem of flatTocItems) {
    ...
    try {
      ...
      const spineItem = bookEpub.spine.spineItems.find(
        (item) =>
          item.href === cleanHref ||
    ...
    reader.onload = async () => {
      try {
        ...
        try {
          ...
          notifications.show({
            title: "Cover Error",
            message: "Could not load cover image.",
            color: "orange",
          });
        }
...
export const exportBookToEpub = async (bookUuid: string): Promise<boolean> => {
  try {
    ...
    imagesToAdd.forEach((img) => {
      ...
      manifestItems.push(
        `<item id="${img.id}" href="images/${img.fileName}" media-type="${img.mime}"/>`
      );
    });
    ...
    chapterData.forEach((ch, idx) => {
      ...
      navPoints.push(
        `<navPoint id="navPoint-${idx + 1}" playOrder="${idx + 1}"><navLabel><text>${ch.title}</text></navLabel><content src="${fileName}"/></navPoint>`
      );
    });
